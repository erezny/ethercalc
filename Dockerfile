...
RUN su ethercalc npm install
RUN su ethercalc make

USER ethercalc
ENV HOME /home/ethercalc
EXPOSE 8000
CMD ["sh", "-c", "REDIS_HOST=$REDIS_PORT_6379_TCP_ADDR REDIS_PORT=$REDIS_PORT_6379_TCP_PORT pm2 start -x `/home/ethercalc/application/bin/ethercalc` -- --cors && pm2 logs"]
